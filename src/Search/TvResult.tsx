...
import { getSearchTv, IGetTvSearchResult } from "../api";
...
const TvResult = ({search} : {search : string}) => {//구조분해 할당기술에 자료형지정
    ...
    const { data, isLoading } = useQuery<IGetTvSearchResult>(
        [{search}, "tvSearch"],
        () => getSearchTv(search),//영화 검색 api룰 불러와
    );
    console.log(data);
    console.log({search});
        ...
        <SliderRow>
            ...
            >
                {data?.results
                    ...
                    .map((tv) => (//data쓸 필요없고 새로운 인자로 고고
                    <Box
                        layoutId={search + tv.id + ""}//layoutId : string임
                        key={tv.id}
                        whileHover="hover"
                        initial="normal"
                        variants={boxVariants}
                        onClick={() => onBoxClicked(tv.original_title)}
                        transition={
                            { search: "tween" }
                        }
